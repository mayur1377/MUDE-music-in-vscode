...
- **Major optimization**: Reduced extension package size from ~38MB to 4.08MB (89% reduction)
...
- Fixed bundling issues with `youtube-dl-exec` and `node-mpv` by keeping them external (required for binary execution)
